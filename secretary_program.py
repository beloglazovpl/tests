...
def add_new_doc(doc, type, name, number):
  new_doc_number = doc
  new_doc_type = type
  new_doc_owner_name = name
  new_doc_shelf_number = number
  new_doc = {
    "type": new_doc_type,
...
def secretary_program_start():
  ...
  while True:
    ...
    if user_command == 'p':
      owner_name = get_doc_owner_name(input())
      ...
      new_doc_shelf_number = add_new_doc(input('Введите номер'), input('Введите тип'), input('Введите имя'), input('Введите номер полки'))
      print('\nНа полку "{}" добавлен новый документ:'.format(new_doc_shelf_number))
    elif user_command == 'd':
      doc_number, deleted = delete_doc(input())
      if deleted:
